...
import { buyProduct_api,removeFromCart_api, fetchCartItems_api } from "./apis_db";
...
const MyCart = () => {
  ...
  const fetchCartItems = async () => {
    ...
    try {
      const data = await fetchCartItems_api(userId);
  ...
  const submitRatingAndBuy = async () => {
    ...
    try {
      const data = await buyProduct_api(userId, selectedProduct, rating);
      alert(data.message);
  ...
  const removeFromCart = async (productId) => {
    try {
      const response = await removeFromCart_api(userId, productId);
